...
return function(ctx)
    ...
    -- Widget compartilhado (Pinion/Duskwire/TryHard) — backup se main antigo nao tiver
    Nav.makePinionQuestStepSection = function(parent, titleText, layoutOrder, mode, stepKey, withTeleport, withCompleteButton, teleportHandler, questNamespace)
        local hudRefs = Nav._hudRefs
        if not hudRefs then return Instance.new("Frame") end
        questNamespace = questNamespace or "pinion"
        withCompleteButton = withCompleteButton ~= false
        local sectionHeight = 56
        local statusHeight = sectionHeight - 24
        local rightPad = 8
        if withTeleport and withCompleteButton then rightPad = 242
        elseif withCompleteButton then rightPad = 166
        elseif withTeleport then rightPad = 92 end
        ...
        if withTeleport then
            ...
            trackConnection(tpBtn.MouseButton1Click:Connect(function()
                task.spawn(function()
                    if teleportHandler then teleportHandler()
                    else Nav.teleportOnceToMode(mode) end
                end)
            end))
        end
        ...
        if withCompleteButton then
            ...
            trackConnection(doneBtn.MouseButton1Click:Connect(function()
                if questNamespace == "duskwire" then
                    ...
                    if Nav.updateDuskwireCompleteButton then
                        Nav.updateDuskwireCompleteButton(stepKey, ctx.S.duskwire.progress[stepKey] == true)
                    end
                elseif questNamespace == "tryhard" then
                    Nav.markTryhardQuestStepComplete(stepKey)
                    if Nav.updateTryhardCompleteButton then
                        Nav.updateTryhardCompleteButton(stepKey, ctx.S.tryhard.progress[stepKey] == true)
                    end
                elseif stepKey:sub(1, 7) == "heavens" then
                    if Nav.markHeavensStepComplete then Nav.markHeavensStepComplete(stepKey) end
                    if Nav.updatePinionCompleteButton then
                        Nav.updatePinionCompleteButton(stepKey, ctx.S.pinion.heavens[stepKey] == true)
                    end
                else
                    Nav.markPinionQuestStepComplete(stepKey)
                    if Nav.updatePinionCompleteButton then
                        Nav.updatePinionCompleteButton(stepKey, ctx.S.pinion.progress[stepKey] == true)
                    end
                end
            end))
            ...
            Nav.makeQuestUndoButton(section, stepKey, undoButtons, UDim2.new(1, -158, 0, 16), function(key)
                if questNamespace == "duskwire" then Nav.unmarkDuskwireQuestStepComplete(key)
                elseif questNamespace == "tryhard" then Nav.unmarkTryhardQuestStepComplete(key)
                else Nav.unmarkPinionQuestStepComplete(key) end
            end)
        end
        return section
    end
