...
--
-- Mapa de blocos:
--   loader.lua                      → único loadstring (baixa main + define URLs)
--   quests/quest_*.lua              → quest rods (ou URLs via NAVEGACAO_QUEST_URLS)
--   core1a_* / core1b_*            → stats, bestiary, core
--   core2a* / core2b                → equip, compra HUD, farm status
--   core3a* / core3a2* / core3b     → teleporte, compra de varas, autosell
--   hud_*                           → interface e labels
...
local function fetchQuestSource(name)
    local urls = rawget(_G, "NAVEGACAO_QUEST_URLS")
    local url = urls and urls[name]
    if url and url ~= "" then
...
local function loadAllQuestModules()
    ...
    if ok then
        ...
        warn("[navegacao] Algumas quest rods falharam — confira URLs em loader.lua ou pasta quests/.")
    end
    return ok
end
...
Nav.startOscarQuest = function()
    ...
    questState.oscar.started = true
    questState.oscar.redoActive = false
...
Nav.startPinionQuest = function()
    ...
    questState.pinion.started = true
    questState.pinion.redoActive = false
...
Nav.startDuskwireQuest = function()
    local info = Nav.getDuskwireQuestStartInfo()
    if info.completed or info.started then return end
    if not info.canStart then
        print("[navegacao] Duskwire Rod:", info.blockedReason or "Requisitos não atendidos para iniciar.")
        return
    end
    questState.duskwire.started = true
    questState.duskwire.redoActive = false
    if Nav.saveDuskwireQuestProgressToCache then
        Nav.saveDuskwireQuestProgressToCache()
    end
    print("[navegacao] Quest Duskwire Rod iniciada.")
    if updateHudVisuals then updateHudVisuals(true) end
end

Nav.startTryhardQuest = function()
    local info = Nav.getTryhardQuestStartInfo()
    if info.completed or info.started then return end
    if not info.canStart then
        print("[navegacao] TryHard Rod:", info.blockedReason or "Requisitos não atendidos para iniciar.")
        return
    end
    questState.tryhard.started = true
    questState.tryhard.redoActive = false
    if Nav.saveTryhardQuestProgressToCache then
        Nav.saveTryhardQuestProgressToCache()
    end
...
Nav.redoOscarQuest = function()
    questState.oscar.redoActive = true
    questState.oscar.started = true
...
Nav.redoPinionQuest = function()
    for _, key in ipairs(PINION_PROGRESS_KEYS) do
        pinionQuestProgress[key] = false
    end
    for _, key in ipairs(PINION_HEAVENS_PROGRESS_KEYS) do
        pinionHeavensProgress[key] = false
    end
    questState.pinion.started = true
    questState.pinion.redoActive = true
    if Nav.savePinionQuestProgressToCache then
        Nav.savePinionQuestProgressToCache()
    end
...
Nav.redoDuskwireQuest = function()
    for _, key in ipairs(DUSKWIRE_PROGRESS_KEYS) do
        duskwireQuestProgress[key] = false
    end
    questState.duskwire.started = true
    questState.duskwire.redoActive = true
    if Nav.saveDuskwireQuestProgressToCache then
        Nav.saveDuskwireQuestProgressToCache()
    end
...
Nav.redoTryhardQuest = function()
    for _, key in ipairs(TRYHARD_PROGRESS_KEYS) do
        tryhardQuestProgress[key] = false
    end
    questState.tryhard.started = true
    questState.tryhard.redoActive = true
    if Nav.saveTryhardQuestProgressToCache then
        Nav.saveTryhardQuestProgressToCache()
    end
...
Nav.markPinionQuestStepComplete = function(stepKey)
    ...
    if pinionQuestProgress[stepKey] then return end
    if stepKey == "step2" and not pinionQuestProgress.step1 then
        print("[navegacao] Pinion Aria: conclua o Passo 1 antes.")
        return
    end
    if stepKey == "step3" then
        if not pinionQuestProgress.step1 then
            print("[navegacao] Pinion Aria: conclua o Passo 1 antes.")
            return
        end
        if not pinionQuestProgress.step2 and not Nav.hasIsonade() then
...
Nav.markDuskwireQuestStepComplete = function(stepKey)
    if stepKey ~= "step1" and stepKey ~= "step2" and stepKey ~= "step3" then return end
    if duskwireQuestProgress[stepKey] then return end
    if not questState.duskwire.started then
        print("[navegacao] Duskwire Rod: inicie a missão antes.")
        return
    end
    if stepKey == "step2" and not duskwireQuestProgress.step1 then
        print("[navegacao] Duskwire Rod: conclua o Passo 1 antes.")
        return
    end
    if stepKey == "step3" and not duskwireQuestProgress.step2 then
        print("[navegacao] Duskwire Rod: conclua o Passo 2 antes.")
        return
    end
    duskwireQuestProgress[stepKey] = true
    if stepKey == "step3" then
        questState.duskwire.redoActive = false
    end
    if Nav.saveDuskwireQuestProgressToCache then
...
Nav.markTryhardQuestStepComplete = function(stepKey)
    if stepKey ~= "step1" and stepKey ~= "step2" then return end
    if tryhardQuestProgress[stepKey] then return end
    if not questState.tryhard.started then
        print("[navegacao] TryHard Rod: inicie a missão antes.")
        return
    end
    if stepKey == "step2" and not tryhardQuestProgress.step1 then
        print("[navegacao] TryHard Rod: conclua o Passo 1 antes.")
        return
    end
    tryhardQuestProgress[stepKey] = true
...
local function clearPinionProgressFromKey(stepKey)
    local startIdx
    for i, key in ipairs(PINION_QUEST_UNDO_ORDER) do
        if key == stepKey then
            startIdx = i
            break
        end
    ...
    for i = startIdx, #PINION_QUEST_UNDO_ORDER do
        local key = PINION_QUEST_UNDO_ORDER[i]
...
local function clearDuskwireProgressFromKey(stepKey)
    local startIdx
    for i, key in ipairs(DUSKWIRE_QUEST_UNDO_ORDER) do
        if key == stepKey then
            startIdx = i
            break
        end
    ...
    if not startIdx then return false end
    for i = startIdx, #DUSKWIRE_QUEST_UNDO_ORDER do
        duskwireQuestProgress[DUSKWIRE_QUEST_UNDO_ORDER[i]] = false
    end
    return true
end

Nav.unmarkDuskwireQuestStepComplete = function(stepKey)
...
local function clearTryhardProgressFromKey(stepKey)
    local startIdx
    for i, key in ipairs(TRYHARD_QUEST_UNDO_ORDER) do
        if key == stepKey then
            startIdx = i
            break
        end
    ...
    if not startIdx then return false end
    for i = startIdx, #TRYHARD_QUEST_UNDO_ORDER do
        tryhardQuestProgress[TRYHARD_QUEST_UNDO_ORDER[i]] = false
    end
    return true
end

Nav.unmarkTryhardQuestStepComplete = function(stepKey)
...
Nav.allHeavensSteps1To4Done = function()
    return pinionHeavensProgress.heavens1
        and pinionHeavensProgress.heavens2
        and pinionHeavensProgress.heavens3
        and pinionHeavensProgress.heavens4
end

Nav.allHeavensTeleportsDone = function()
    for _, key in ipairs(HEAVENS_STEP5_KEYS) do
        if not pinionHeavensProgress[key] then
            return false
        end
    end
    return true
...
Nav.markHeavensStepComplete = function(stepKey)
    ...
    if pinionHeavensProgress[stepKey] then return end
    ...
    pinionHeavensProgress[stepKey] = true
    if Nav.savePinionQuestProgressToCache then
        Nav.savePinionQuestProgressToCache()
    end
...
Nav.makePinionHeavensStep5Section = function(parent, layoutOrder)
    local rowHeight = 32
    local headerHeight = 22
    local sectionHeight = headerHeight + (#PINION_HEAVENS_ROD_SPOTS * rowHeight) + 8
    ...
    for i, spot in ipairs(PINION_HEAVENS_ROD_SPOTS) do
        local mode = PINION_HEAVENS_MODE_LIST[i]
...
Nav.makePinionHeavensStep6Section = function(parent, layoutOrder)
    local mode = MODES.PINION_HEAVENS_6
    ...
    status.Text = string.format(
        "Teleportar — %.2f, %.2f, %.2f",
        PINION_HEAVENS_FINAL_SPOT.X,
        PINION_HEAVENS_FINAL_SPOT.Y,
...
Nav.updateOscarQuestLabels = function()
    ...
    if questState.oscar.redoActive then
...
Nav.updatePinionQuestLabels = function()
    ...
    local questsActive = (pinionStarted and not pinion.completed) or questState.pinion.redoActive
    ...
    if not pinion.completed and not pinionStarted then
        ...
        elseif startInfo.canStart then
            overviewLabel = string.format("Requisitos OK — level %d+", PINION_ARIA_MIN_LEVEL) .. "\nClique Iniciar para ver os passos"
            overviewColor = Color3.fromRGB(80, 200, 100)
        end
    end
    ...
    if questState.pinion.redoActive and not pinion.completed then
        questState.pinion.redoActive = false
    end
    ...
    Nav.setLabelIfChanged(
        ...
        pinion.step1Done
            ...
            or string.format(
                ...
                PINION_VERTIGO_UNLOCK_SPOT.X,
                PINION_VERTIGO_UNLOCK_SPOT.Y,
                PINION_VERTIGO_UNLOCK_SPOT.Z
            ),
    ...
    if showStep4 then
        Nav.setLabelIfChanged(
            ...
            string.format(
                ...
                PINION_DEPTHS_KEY_SPOT.Y,
                PINION_DEPTHS_KEY_SPOT.Z
            ),
            Color3.fromRGB(170, 178, 195)
        )
    end
    ...
    if showStep5 then
        Nav.setLabelIfChanged(
            ...
            string.format(
                ...
                PINION_ENCHANT_RELIC_SPOT.Y,
                PINION_ENCHANT_RELIC_SPOT.Z
            ),
            Color3.fromRGB(170, 178, 195)
        )
    end
    ...
    if showHeavens5 then
        for i, mode in ipairs(PINION_HEAVENS_MODE_LIST) do
    ...
    if showStep10 then
        Nav.setLabelIfChanged(
            ...
            pinion.step10Done
                ...
                or string.format(
                    "Teleportar — %.2f, %.2f, %.2f",
                    PINION_HEAVENLY_DOVE_SPOT.X,
                    PINION_HEAVENLY_DOVE_SPOT.Y,
                    PINION_HEAVENLY_DOVE_SPOT.Z
                ),
    ...
    if showStep12 then
        Nav.setLabelIfChanged(
            ...
            pinion.step12Done
                ...
                or string.format(
                    "Ir pescar Dj Spinous — teleporte: %.2f, %.2f, %.2f",
                    PINION_POST_QUEST_FISH_SPOT.X,
                    PINION_POST_QUEST_FISH_SPOT.Y,
                    PINION_POST_QUEST_FISH_SPOT.Z
                ),
...
Nav.updateDuskwireQuestLabels = function()
    local duskwire = Nav.getDuskwireQuestAvailability()
    local startInfo = Nav.getDuskwireQuestStartInfo()
    local duskwireStarted = startInfo.started == true
    local questsActive = (duskwireStarted and not duskwire.completed) or questState.duskwire.redoActive
    ...
    if not duskwire.completed and not duskwireStarted then
        if startInfo.blockedReason then
            ...
            overviewColor = Color3.fromRGB(170, 178, 195)
        elseif startInfo.canStart then
            overviewLabel = string.format("Requisitos OK — level %d+", DUSKWIRE_ROD_MIN_LEVEL) .. "\nClique Iniciar para ver os passos"
            overviewColor = Color3.fromRGB(80, 200, 100)
        end
    end
    ...
    if questState.duskwire.redoActive and not duskwire.completed then
        questState.duskwire.redoActive = false
    end
    ...
    Nav.updateDuskwireCompleteButton("step1", duskwire.step1Done == true)
    Nav.updateDuskwireCompleteButton("step2", duskwire.step2Done == true)
    Nav.updateDuskwireCompleteButton("step3", duskwire.step3Done == true)
    ...
    if showStep1 then
        Nav.setLabelIfChanged(
            ...
            duskwire.step1Done
                ...
                or string.format(
                    "Teleportar — %.2f, %.2f, %.2f",
                    DUSKWIRE_STEP1_SPOT.X,
                    DUSKWIRE_STEP1_SPOT.Y,
                    DUSKWIRE_STEP1_SPOT.Z
                ),
        ...
        Nav.setLabelIfChanged(
            ...
            duskwire.step2Done
                ...
                or string.format(
                    ...
                    DUSKWIRE_STEP2_SPOT.X,
                    DUSKWIRE_STEP2_SPOT.Y,
                    DUSKWIRE_STEP2_SPOT.Z
                ),
...
Nav.updateTryhardQuestLabels = function()
    local tryhard = Nav.getTryhardQuestAvailability()
    local startInfo = Nav.getTryhardQuestStartInfo()
    local tryhardStarted = startInfo.started == true
    local questsActive = (tryhardStarted and not tryhard.completed) or questState.tryhard.redoActive
    ...
    if questState.tryhard.redoActive and not tryhard.completed then
        questState.tryhard.redoActive = false
    end
    ...
    Nav.updateTryhardCompleteButton("step1", tryhard.step1Done == true)
    Nav.updateTryhardCompleteButton("step2", tryhard.step2Done == true)
    ...
    if showStep1 then
        Nav.setLabelIfChanged(
            ...
            tryhard.step1Done
                ...
                or string.format(
                    "Teleportar — %.2f, %.2f, %.2f",
                    TRYHARD_STEP1_SPOT.X,
                    TRYHARD_STEP1_SPOT.Y,
                    TRYHARD_STEP1_SPOT.Z
                ),
        ...
        Nav.setLabelIfChanged(
            ...
            tryhard.step2Done
                ...
                or string.format(
                    ...
                    TRYHARD_STEP2_SPOT.X,
                    TRYHARD_STEP2_SPOT.Y,
                    TRYHARD_STEP2_SPOT.Z
                ),
...
Nav.updateQuestRodsTabLabels = function()
    ...
    Nav.updateDuskwireQuestLabels()
    Nav.updateTryhardQuestLabels()
end
